...
This blog lays out ideas for designing your AI application such that model and system prompts are decoupled from the underlying implementation, which in turn makes it easier to iterate on.

---

## Model

Say you want to integrate a mix of local (e.g. Ollama) and hosted (OpenAI, Anthropic, etc.) models in your application. 
...
More practically, I decoupled the system prompt by templatising them using [Jinja2](https://jinja.palletsprojects.com/en/stable/), and moving it into its own directory. Each prompt could then be loaded in as desired by utilising a helper function.
...
There are many approaches I could have taken to achieve a similar outcome, but the key idea is to abstract out the model and system prompts so that they can be easily iterated on.

This is especially important when we're at point in time where it feels like new models and techniques are coming out every second.

I ran DeepEval against the combination I mentioned above, and the results were as below:
...
I don’t think it’s that surprising that `gpt-4-turbo` outperforms `gemma3:4b`, but I found it interesting to see that the `02.jinja` prompt which performed better on the OpenAI model didn’t perform as well on `gemma3:4b`; meaning we can’t choose to optimize for either of model or system prompt in isolation.

**My take is**: the `02.jinja` prompt contains more granularity, which a larger model (like `gpt-4-turbo`) is able to better interpret and make sense of. Something to look into next...
